	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mDrawerList.setOnItemClickListener(new OnItemClickListener() {
	        ...
	        public void onItemClick(AdapterView<?> parent, View view, int position, long id){
		    	...
		    	SharedPreferences prefs = MainActivity.this.getSharedPreferences(Utils.PREFERENCES, 0);
		    	SharedPreferences.Editor editor = prefs.edit();
		        editor.putInt("selectedCategory", categories.get(position).getId());
		        editor.putString("categoryName", categories.get(position).getName());
		        editor.commit();
				startActivity(intent);
			}
	    });
    	SharedPreferences prefs = this.getSharedPreferences(Utils.PREFERENCES, 0);
    	SharedPreferences.Editor editor = prefs.edit();
	...
	public void onReceiveResult(int resultCode, Bundle resultData) {
		...
		case ApiService.STATUS_FINISHED:
			...
			if(resultData.getString(Utils.METHOD_CLASS).equals("ar.edu.itba.model.GetAllProducts")) {
            	...
            	ProductAdapter imageAdapter = new ProductAdapter(this, response.getProducts());
